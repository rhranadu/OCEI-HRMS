...
class HolidayCalendarController extends Controller
{
    ...
    public function create(){
        ...
        return view('admin.leave.holidayCalendar.form');
    }
    ...
    public function update(HolidayCalendarRequest $request,$id) {
        ...
        $holidayDetails     = HolidayCalendar::findOrFail($id);
        $input              = $request->all();
        ...
        if ($file) {
           ...
           if($ext != 'jpeg' and $ext != 'png') {
            ...
            $name = $file->getClientOriginalName();
            $destination = public_path().'/uploads/calendarImg';
            // dd($destination);
        ...
        try{
         ...
         // dd($holidayDetails);
        if($bug==0){
            return redirect('holidayCalendar')->with('success', 'Public holiday successfully updated. ');
        }else {
            return redirect('holidayCalendar')->with('error', 'Something Error Found !, Please try again.');
        }
    }
